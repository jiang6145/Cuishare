...
import { articleFileStorage, userFileStorage, uploadHandler } from '../middlewares/fileStorage.js'
...
export const articleImageUpload = async (req, res, next) => {
  try {
    ...
    const storage = articleFileStorage()
...
export const deleteArticleImage = async (req, res, next) => {
  try {
    if (!req.session.user) return res.status(401).send({ success: false, message: '未登入' })
...
// 使用者照片
export const userPhotoUpload = async (req, res, next) => {
  try {
    if (!req.session.user) return res.status(401).send({ success: false, message: '未登入' })

    const storage = userFileStorage(req.session.user._id)
    const upload = uploadHandler(storage)
